...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications = np.loadtxt("C://xampp/htdocs/TESTFY/classifications.txt", np.float32)                  # read in training classifications
    ...
    try:
        npaFlattenedImages = np.loadtxt("C:/xampp/htdocs/TESTFY/flattened_images.txt", np.float32)                 # read in training images
    except:                                                                                 # if file could not be opened
...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    if len(listOfPossiblePlates) == 0:          # if list of possible plates is empty  #cant locate car plate
    ...
    for possiblePlate in listOfPossiblePlates:          # for each possible plate, this is a big for loop that takes up most of the function
        ...
        possiblePlate.imgThresh = cv2.resize(possiblePlate.imgThresh, (0, 0), fx = 1.6, fy = 1.6)
                ...
                # threshold again to eliminate any gray areas #THRESH_BINARY（黑白二值),Otsu’s就可以自己找到一个认为最好的阈值
        ...
        # end if # show steps #####################################################################
                ...
                # this function first finds all contours(輪廓), then only includes contours(輪廓) that could be chars (without comparison to other chars yet)
        ...
        if Main.showSteps == True: # show steps ###################################################
            ...
            for listOfMatchingChars in listOfListsOfMatchingCharsInPlate:
                ...
                # end for
                cv2.drawContours(imgContours, contours, -1, (intRandomBlue, intRandomGreen, intRandomRed)) #draw all countour
            # end for
        ...
        # end for
                ...
                # 經過preprocess后，找出輪廓contour一樣的，不一樣的排除
...
def checkIfPossibleChar(possibleChar):
            ...
            #char的大小檢查可能性
            ##MIN_PIXEL_AREA=80 面積
            # ##MIN_PIXEL_WIDTH=2 寬
            # ##MIN_PIXEL_HEIGHT=8 高
            # ##MIN_ASPECT_RATIO=0.25 最小比例 
            # ##MAX_ASPECT_RATIO=1.0 最大比例
            #每個contour對比全部contour 
...
def findListOfMatchingChars(possibleChar, listOfChars):
    ...
    for possibleMatchingChar in listOfChars:                # for each char in big list
        ...
        fltChangeInHeight = float(abs(possibleMatchingChar.intBoundingRectHeight - possibleChar.intBoundingRectHeight)) / float(possibleChar.intBoundingRectHeight)
                ...
                #檢查chars之間距離，角度，面積和寬高，距離需少過MAX_DIAG_SIZE，面積角度大小不能變化太多
...
###################################################################################################
# use Pythagorean theorem to calculate distance between two chars #彼氏定理
def distanceBetweenChars(firstChar, secondChar):
    intX = abs(firstChar.intCenterX - secondChar.intCenterX)  #abs=absolute值  
...
def angleBetweenChars(firstChar, secondChar):
    fltAdj = float(abs(firstChar.intCenterX - secondChar.intCenterX)) ###找到X寬度
    fltOpp = float(abs(firstChar.intCenterY - secondChar.intCenterY)) ###找到Y寬度
